...
public class WebConfig implements WebMvcConfigurer {
	...
	public void configureContentNegotiation(ContentNegotiationConfigurer configurer) {
		...
		// Pass the content type parameters via url
		// Example: localhost:8080/api/person/v1?mediaType=xml
		/*
		...
		// Pass the content type via Header (key: Accept, Value: application/xml
		configurer.favorParameter(false).ignoreAcceptHeader(false).useRegisteredExtensionsOnly(false)
				...
				.mediaType("xml", MediaType.APPLICATION_XML).mediaType("x-yaml", APPLICATION_YML);
	}
